...
class PanelVisitados extends JPanel {
	...
	private final Dimension DIM_PANEL_PRINCIPAL = new Dimension(120, 120);
	private final Dimension DIM_PANEL_FOTOS = new Dimension(120, 120);
	private final Dimension DIM_LABEL = new Dimension(100, 100);
	private final Color COLOR_PANEL_FOTOS = Color.white;
	private final Color COLOR_BORDE_PANEL_FOTOS = Color.black;

	/** Constructores **/

	public PanelVisitados(Galeria galeria, VentanaPpal v) {
		...
		for (int i = 0; i < LENGTH_LABELS; i++) {
			...
			panel[i].setBorder(BorderFactory.createLineBorder(
					COLOR_BORDE_PANEL_FOTOS, 2));
				...
				public void mouseClicked(MouseEvent e) {
					...
					for (int j = 0; j < LENGTH_LABELS && !encontrado; j++)
						if (e.getSource() == label[j]) {
							...
							// actualizarPanel(); No hay nada que actualizar en
							// este panel
							if (dV != null)
	...
	/*
	 * public ArrayList<ImageIcon> getImagenes() { return galeria.getFotos(); }
	 */
	...
	/**
	 * @param index
	 * @return si el índice es correcto, devuelve el elemento que NO quita de la
	 *         lista.
	 */
	public DescripcionVivienda getDescVivienda(int index) {
		if (index >= 0 && index < visitadas.size())
			// return vistas.remove(index); -> Por que haces un remove?? no
			// entiendo
	...
	/**
	 * Sirve para actualizar el panel a las últimas casas visitadas por el
	 * usuario
	 */
	public void actualizarPanel() {
		...
		if (visitadas.size() > LENGTH_LABELS)
			...
			actualizarToolTip(visitadas.size() - 1);
	}

	/* AUXILIARES */

	/**
	 * Esto sólo es para ver en la consola el contenido del ArrayList y depurar
	 * más rápido.
	 */
	void mostrarViviendas() {
		for (Iterator<DescripcionVivienda> it = visitadas.iterator(); it
				.hasNext();)
	...
	private void actualizarToolTip(int index) {
		...
		// if (visitadas.get(imagen[index]).getId() !=
		// galeria.getIdIcon((ImageIcon)label[index].getIcon())) {
		if (imagen[index] >= 0) {
