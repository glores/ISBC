...
class VentanaResult extends JDialog {
	...
	private ArrayList<DescripcionVivienda> aL;
	private JLabel[] imagen;
	private final double TOPE_FIJO = 70;
	private final double WIDTH_FIJO = 1130;
	...
	public VentanaResult(Galeria galeria) {
		...
		/**
		 * Colores html = {aqua, black, blue, fuchsia, gray, green, lime,
	...
	public void setResultado(ArrayList<DescripcionVivienda> descrs) {
		...
		for (int i = 0; i < aL.size(); i++) {
			...
			// luego. La localizaciÃ³n no la actualizamos por si tenemos que
			// usar la ruta del Ã¡rbol mÃ¡s adelante.
			...
			String descr = "<html><p align=\"justify\">"
					...
					+ " â‚¬<br>" + "<b><u>DESCRIPCIÃ“N</u></b>: " + descripcion
					+ "</p></html>";
			...
			int pos = galeria.addFoto(descrs.get(i).getUrlFoto(), descrs.get(i)
					.getId());
			// La cargamos:
			imagen[i].setIcon(galeria.getFotoOrigPos(pos));
			imagen[i].setBorder(BorderFactory
					.createBevelBorder(BevelBorder.LOWERED));
					...
					for (int j = 0; j < aL.size() && !encontrado; j++)
						if (e.getSource() == imagen[j]) {
							if (!idYaVisitadas.contains(aL.get(j).getId()))
								...
								VentanaPpal.addVivienda(aL.get(j));
