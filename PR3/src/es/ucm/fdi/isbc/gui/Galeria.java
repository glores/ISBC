...
/** Métodos **/
	...
	/* Getters */
		...
		{
			//Puede que haga falta tratamiento para las null.
			return fotos;
		}*/
		
		/**
		 * Devolvemos todos los IDs que aparecen en la galeria, es decir, todas las fotos
		 * que tenemos dentro de la galeria ya cargadas
		 * @return
		 */
		 ...
		 * @return
		 */
		public ImageIcon getFotoPos(int pos)
		{  //Devolvemos su imagen correspondiente
			if(pos < fotos.size())
			{
				ImageIcon i = fotos.get(pos);
				if(i == null) 
		...
		/**Dada una imagen, devuelve su id correspondiente Importante: usa la descripcion
		 * de la imagen para comparar
		 * 
		 * @param img
		 * @return el id
		 */
		...
		{					
			...
			else return -1; //No se encontró la foto y por tanto el id tmp
		}
	...
	/* Setters */
		...
		/**
		 * Añade una foto a partir de una url, devuelve la posicion donde se guardó la foto
		 * @param urlfoto
		 * @param id
		 * @return devuelve la posición en la que se ha insertado la foto
		 */
	...
	/* Otros métodos */
		...
		{
			if(width == 0) width = SCALED_WIDTH;
			if(heigth == 0) heigth = SCALED_HEIGTH;
			return new ImageIcon(origin.getImage().getScaledInstance(width, heigth, Image.SCALE_FAST));
		}
