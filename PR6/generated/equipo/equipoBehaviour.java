//Template for Behaviour file
//This file is used by code generator
package equipo;

//IMPORT SECTION 
import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;
import java.util.Queue;
import java.util.Vector;

import EDU.gatech.cc.is.util.Vec2;
import teams.ucmTeam.Behaviour;
import teams.ucmTeam.Message;
import teams.ucmTeam.RobotAPI;

public class equipoBehaviour extends Behaviour {
	/**
	 * Table for manage all state of the HFMS
	 */
	private Vector<State> states;
	/**
	 * Stack for manage the current state and the hierarchical child state of the current state  
	 */
	private Vector<State> stack;
	
	
	@Override
	public void configure() {
		//Call Generate the list of states from HFSM
		GenerateStates();
	}

	/**
	 * Put the code that execute on each iteration of simulation
	 */
	@Override
	public int takeStep() {
		try 
		{
			this.getClass().getMethod(stack.lastElement().actions, (Class[]) null).invoke(this, (Object[]) null);
		} 
		catch (IllegalArgumentException ex) {
			ex.printStackTrace();
		} catch (SecurityException ex) {
			ex.printStackTrace();
		} catch (IllegalAccessException ex) {
			ex.printStackTrace();
		} catch (InvocationTargetException ex) {
			ex.printStackTrace();
		} catch (NoSuchMethodException ex) {
			ex.printStackTrace();
		}
		try 
		{
			int positionStack = 0;
			while(positionStack < stack.size())
			{
				State e = stack.get(positionStack);
				if (e.transitions != null) 
				{
					for (int j = 0; j < e.transitions.length; j++) 
					{
						if ((Boolean) this.getClass().getMethod(e.transitions[j].condition, (Class[]) null).invoke(this, (Object[]) null)) 
						{
							// Remove state of the stack 
							for (int k = stack.size() - 1; k >= positionStack; k--) {
								stack.remove(k);
							}
							// Put new state over the stack
							stack.add(states.get(e.transitions[j].state));
						}
					}
				}
				positionStack++;
			}
		} catch (IllegalArgumentException ex) {
			ex.printStackTrace();
		} catch (SecurityException ex) {
			ex.printStackTrace();
		} catch (IllegalAccessException ex) {
			ex.printStackTrace();
		} catch (InvocationTargetException ex) {
			ex.printStackTrace();
		} catch (NoSuchMethodException ex) {
			ex.printStackTrace();
		}
		
		return myRobotAPI.ROBOT_OK;
	}

	@Override
	public void onInit(RobotAPI r) {
		//Initialize the stack with the first state 
		stack = new Vector<State>();
		stack.add(states.get(0));
	}

	@Override
	public void end() {	}

	@Override
	public void onRelease(RobotAPI r) { }
	
	//Create the table of State. It has been generated by code generator 
	private void GenerateStates()
	{
		states = new Vector<State>();
		State e;
		
		//State 0 : Node  1 
		e = new State();
		e.transitions = new Transition[3];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_0_0";
		e.transitions[0].state = 1;
		e.transitions[1] = new Transition();
		e.transitions[1].condition = "cond_0_1";
		e.transitions[1].state = 2;
		e.transitions[2] = new Transition();
		e.transitions[2].condition = "cond_0_2";
		e.transitions[2].state = 3;
		e.actions = "acc_0";
		states.add(e);

		//State 1 : Node 239 
		e = new State();
		e.transitions = null;
		e.actions = "acc_1";
		states.add(e);

		//State 2 : Node 191 
		e = new State();
		e.transitions = null;
		e.actions = "acc_2";
		states.add(e);

		//State 3 : Node 201 
		e = new State();
		e.transitions = null;
		e.actions = "acc_3";
		states.add(e);

		//State 4 : Node 230 
		e = new State();
		e.transitions = new Transition[1];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_4_0";
		e.transitions[0].state = 7;
		e.actions = "acc_4";
		states.add(e);

		//State 5 : Node 180 
		e = new State();
		e.transitions = new Transition[1];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_5_0";
		e.transitions[0].state = 8;
		e.actions = "acc_5";
		states.add(e);

		//State 6 : Node 198 
		e = new State();
		e.transitions = new Transition[1];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_6_0";
		e.transitions[0].state = 9;
		e.actions = "acc_6";
		states.add(e);

		//State 7 : Node 228 
		e = new State();
		e.transitions = new Transition[3];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_7_0";
		e.transitions[0].state = 4;
		e.transitions[1] = new Transition();
		e.transitions[1].condition = "cond_7_1";
		e.transitions[1].state = 10;
		e.transitions[2] = new Transition();
		e.transitions[2].condition = "cond_7_2";
		e.transitions[2].state = 11;
		e.actions = "acc_7";
		states.add(e);

		//State 8 : Node 181 
		e = new State();
		e.transitions = new Transition[2];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_8_0";
		e.transitions[0].state = 12;
		e.transitions[1] = new Transition();
		e.transitions[1].condition = "cond_8_1";
		e.transitions[1].state = 5;
		e.actions = "acc_8";
		states.add(e);

		//State 9 : Node 197 
		e = new State();
		e.transitions = new Transition[1];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_9_0";
		e.transitions[0].state = 6;
		e.actions = "acc_9";
		states.add(e);

		//State 10 : Node 231 
		e = new State();
		e.transitions = new Transition[1];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_10_0";
		e.transitions[0].state = 7;
		e.actions = "acc_10";
		states.add(e);

		//State 11 : Node 229 
		e = new State();
		e.transitions = new Transition[2];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_11_0";
		e.transitions[0].state = 7;
		e.transitions[1] = new Transition();
		e.transitions[1].condition = "cond_11_1";
		e.transitions[1].state = 4;
		e.actions = "acc_11";
		states.add(e);

		//State 12 : Node 175 
		e = new State();
		e.transitions = new Transition[2];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_12_0";
		e.transitions[0].state = 5;
		e.transitions[1] = new Transition();
		e.transitions[1].condition = "cond_12_1";
		e.transitions[1].state = 8;
		e.actions = "acc_12";
		states.add(e);

		//State 13 : Node 183 
		e = new State();
		e.transitions = new Transition[1];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_13_0";
		e.transitions[0].state = 15;
		e.actions = "acc_13";
		states.add(e);

		//State 14 : Node 177 
		e = new State();
		e.transitions = new Transition[1];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_14_0";
		e.transitions[0].state = 16;
		e.actions = "acc_14";
		states.add(e);

		//State 15 : Node 182 
		e = new State();
		e.transitions = new Transition[1];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_15_0";
		e.transitions[0].state = 13;
		e.actions = "acc_15";
		states.add(e);

		//State 16 : Node 176 
		e = new State();
		e.transitions = new Transition[1];
		e.transitions[0] = new Transition();
		e.transitions[0].condition = "cond_16_0";
		e.transitions[0].state = 14;
		e.actions = "acc_16";
		states.add(e);


	}
	
	//Conditions and actions that was building based in the graph of de HFMS of the tool editor
	//Conditions for State 0 : Node  1
	public Boolean cond_0_0(){
		return ((myRobotAPI.getPlayerNumber() == 1) || (myRobotAPI.getPlayerNumber() == 2));
	}

//Conditions for State 0 : Node  1
	public Boolean cond_0_1(){
		return ((myRobotAPI.getPlayerNumber() == 0));
	}

//Conditions for State 0 : Node  1
	public Boolean cond_0_2(){
		return ((myRobotAPI.getPlayerNumber() == 3) || (myRobotAPI.getPlayerNumber() == 4));
	}

	public void acc_0(){
	}

	public void acc_1(){
stack.add(states.get(4));
	}

	public void acc_2(){
stack.add(states.get(5));
	}

	public void acc_3(){
stack.add(states.get(6));
	}

//Conditions for State 4 : Node 230
	public Boolean cond_4_0(){
		return ((-myRobotAPI.getFieldSide() * myRobotAPI.getPosition().x < 0) && (-myRobotAPI.getFieldSide() * myRobotAPI.toFieldCoordinates(myRobotAPI.getBall()).x < 0) && (myRobotAPI.getBall().r < 0.3));
	}

	public void acc_4(){
		Iralapelota();
	}

//Conditions for State 5 : Node 180
	public Boolean cond_5_0(){
		return ((myRobotAPI.getBall().r < 0.3));
	}

	public void acc_5(){
		CoverGoal();
	}

//Conditions for State 6 : Node 198
	public Boolean cond_6_0(){
		return ((myRobotAPI.canKick() == false));
	}

	public void acc_6(){
		Patear();
	}

//Conditions for State 7 : Node 228
	public Boolean cond_7_0(){
		return ((-myRobotAPI.getFieldSide() * myRobotAPI.getPosition().x < 0) && (-myRobotAPI.getFieldSide() * myRobotAPI.toFieldCoordinates(myRobotAPI.getBall()).x < 0) && (myRobotAPI.getBall().r >= 0.3));
	}

//Conditions for State 7 : Node 228
	public Boolean cond_7_1(){
		return ((-myRobotAPI.getFieldSide() * myRobotAPI.getPosition().x >= 0) && (-myRobotAPI.getFieldSide() * myRobotAPI.toFieldCoordinates(myRobotAPI.getBall()).x < 0) && (myRobotAPI.getBall().r >= 0.3));
	}

//Conditions for State 7 : Node 228
	public Boolean cond_7_2(){
		return ((-myRobotAPI.getFieldSide() * myRobotAPI.getPosition().x < 0) && (-myRobotAPI.getFieldSide() * myRobotAPI.toFieldCoordinates(myRobotAPI.getBall()).x >= 0) && (myRobotAPI.getBall().r >= 0.3));
	}

	public void acc_7(){
		Posicionarypatear();
	}

//Conditions for State 8 : Node 181
	public Boolean cond_8_0(){
		return ((-myRobotAPI.getFieldSide() * myRobotAPI.toFieldCoordinates(myRobotAPI.getBall()).x >= 0) && !((myRobotAPI.getPosition().y < 0.5) && (myRobotAPI.getPosition().y > -0.5) && (-myRobotAPI.getFieldSide() * myRobotAPI.getPosition().x < -1.145)));
	}

//Conditions for State 8 : Node 181
	public Boolean cond_8_1(){
		return ((-myRobotAPI.getFieldSide() * myRobotAPI.toFieldCoordinates(myRobotAPI.getBall()).x < 0) && (myRobotAPI.getBall().r >= 0.3));
	}

	public void acc_8(){
stack.add(states.get(13));
	}

//Conditions for State 9 : Node 197
	public Boolean cond_9_0(){
		return ((myRobotAPI.canKick() == true));
	}

	public void acc_9(){
		Iralapelota();
	}

//Conditions for State 10 : Node 231
	public Boolean cond_10_0(){
		return ((-myRobotAPI.getFieldSide() * myRobotAPI.getPosition().x < 0) && (myRobotAPI.getBall().r >= 0.3));
	}

	public void acc_10(){
		Andarhaciaelcampo();
	}

//Conditions for State 11 : Node 229
	public Boolean cond_11_0(){
		return ((-myRobotAPI.getFieldSide() * myRobotAPI.getPosition().x < 0) && (-myRobotAPI.getFieldSide() * myRobotAPI.toFieldCoordinates(myRobotAPI.getBall()).x >= 0) && (myRobotAPI.getBall().r < 0.3));
	}

//Conditions for State 11 : Node 229
	public Boolean cond_11_1(){
		return ((-myRobotAPI.getFieldSide() * myRobotAPI.getPosition().x < 0) && (-myRobotAPI.getFieldSide() * myRobotAPI.toFieldCoordinates(myRobotAPI.getBall()).x < 0) && (myRobotAPI.getBall().r >= 0.3));
	}

	public void acc_11(){
		Wait();
	}

//Conditions for State 12 : Node 175
	public Boolean cond_12_0(){
		return ((-myRobotAPI.getFieldSide() * myRobotAPI.toFieldCoordinates(myRobotAPI.getBall()).x < 0) && (myRobotAPI.getPosition().y > -0.5) && (myRobotAPI.getPosition().y < 0.5) && (-myRobotAPI.getFieldSide() * myRobotAPI.getPosition().x < -1.145));
	}

//Conditions for State 12 : Node 175
	public Boolean cond_12_1(){
		return ((-myRobotAPI.getFieldSide() * myRobotAPI.toFieldCoordinates(myRobotAPI.getBall()).x < 0) && (myRobotAPI.getBall().r < 0.3));
	}

	public void acc_12(){
stack.add(states.get(14));
	}

//Conditions for State 13 : Node 183
	public Boolean cond_13_0(){
		return ((myRobotAPI.canKick() == false));
	}

	public void acc_13(){
		Patear();
	}

//Conditions for State 14 : Node 177
	public Boolean cond_14_0(){
		return ((myRobotAPI.getOurGoal().r > 0.1));
	}

	public void acc_14(){
		Wait();
	}

//Conditions for State 15 : Node 182
	public Boolean cond_15_0(){
		return ((myRobotAPI.canKick() == true));
	}

	public void acc_15(){
		Iralapelota();
	}

//Conditions for State 16 : Node 176
	public Boolean cond_16_0(){
		return ((myRobotAPI.getOurGoal().r <= 0.1));
	}

	public void acc_16(){
		WalkTowardsGoal();
	}


	
	//Code of basic behaviours that was building based in basic behaviours of de game model
		public void LeadBallToGoal() { 

		myRobotAPI.alignedToBallandGoal();
myRobotAPI.kick();

	}
 	public void BlockForward() { 

		myRobotAPI.blockForward();

	}
 	public void GoToCenter() { 

		Vec2 destino = new Vec2(0.0, 0.0);
destino.sub(myRobotAPI.getPosition());
myRobotAPI.setSteerHeading(destino.t);
myRobotAPI.setSpeed(1.0);

	}
 	public void Posicionarypatear() { 

		// Vec2 EGOCÉNTRICO que apunta hacia la pelota
Vec2 destination = myRobotAPI.getBall();
// Posicionamos al robot
myRobotAPI.setBehindBall(destination);
// Ponemos la velocidad
myRobotAPI.setSpeed(1.0);

	}
 	public void Iralapelota() { 

		// Sacamos el ángulo de la pelota
double angle = myRobotAPI.getBall().t;
// Cambiamos el ángulo del robot
myRobotAPI.setSteerHeading(angle);
// Ponemos la velocidad
myRobotAPI.setSpeed(1.0);

	}
 	public void Unblock() { 

		myRobotAPI.avoidCollisions();

	}
 	public void BlockClosest() { 

		 

	}
 	public void CoverGoal() { 

		Vec2 dest = new Vec2(myRobotAPI.getOurGoal());
dest.add(myRobotAPI.getBall());
myRobotAPI.setSteerHeading(dest.t);
myRobotAPI.setSpeed(1.0);

	}
 	public void Andarhaciaelcampo() { 

		 

	}
 	public void Wait() { 

		myRobotAPI.setSpeed(0.0);

	}
 	public void Patear() { 

		myRobotAPI.kick();

	}
 	public void WalkTowardsGoal() { 

		myRobotAPI.setSteerHeading(myRobotAPI.getOurGoal().t);
myRobotAPI.setSpeed(1.0);

	}
 	public void BlockGoalkeeper() { 

		myRobotAPI.blockGoalKeeper();

	}
 
	
	/**
	 * Strut to define a Transition of the HFMS
	 * @author Jorge
	 *
	 */
	
	private class Transition {
		String condition;
		int state;
	}
	/**
	 * Struct to define a State of the HFMS
	 * @author Jorge
	 *
	 */
	private class State {
		Transition[] transitions;
		String actions;
	}
	
	/**
	 * The equals method will be used to check if two behaviours are equal (doh).
	 * 
	 * When a robot is executing a behaviour in SBT and then the TM decides to change
	 * it for another, this method will be consulted prior to change. When behaviours 
	 * change they are reinitialized (the state machine goes back to the initial state).
	 * 
	 */
	@Override
	public boolean equals(Object obj) {
		if (obj == null)
			return false;
		return obj.getClass().equals(getClass());
	}

}